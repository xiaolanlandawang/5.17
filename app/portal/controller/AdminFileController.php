...
class AdminFileController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $data        = $postService->adminArticleList($param,4);

        $data->appends($param);

        $portalCategoryModel = new PortalCategoryModel();
        $categoryTree        = $portalCategoryModel->adminCategoryTree(4, $categoryId);
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            $portalPostModel->adminAddPage($data['post'], 4);


            $this->success('添加成功!', url('AdminFile/index'));
        }
    ...
    public function editPost()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $portalPostModel->adminEditPage($data['post'], 4);
