...
class AdminNewsCategoryController extends AdminBaseController
{
    /**
     * 文章分类列表
     * @adminMenu(
    ...
    public function index()
    {
        ...
        if (empty($keyword)) {
            $categoryTree = $portalCategoryModel->adminCategoryTableTree(8);
    ...
    public function add()
    {
        ...
        $portalCategoryModel = new PortalCategoryModel();
        $categoriesTree      = $portalCategoryModel->adminCategoryTree(8, $parentId);
    ...
    public function addPost()
    {
        ...
        if (!empty($data['alias']) && !empty($id)) {
            $routeModel->setRoute($data['alias'], 'portal/index/news', ['id' => $id], 2, 5000);
            $routeModel->setRoute($data['alias'] . '/:id', 'portal/index/news_info', ['cid' => $id], 2, 4999);
        }
        $routeModel->getRoutes(true);

        $this->success('添加成功!', url('AdminNewsCategory/index'));
    }
    ...
    public function edit()
    {
        ...
        if ($id > 0) {
            ...
            $categoriesTree = $portalCategoryModel->adminCategoryTree(3, $category['parent_id'], $id);
            ...
            $routeModel = new RouteModel();
            $alias      = $routeModel->getUrl('portal/index/news', ['id' => $id]);
    ...
    public function editPost()
    {
        ...
        $routeModel = new RouteModel();
        if (!empty($data['alias'])) {
            $routeModel->setRoute($data['alias'], 'portal/index/news', ['id' => $data['id']], 2, 5000);
            $routeModel->setRoute($data['alias'] . '/:id', 'portal/index/news_info', ['cid' => $data['id']], 2, 4999);
        } else {
            $routeModel->deleteRoute('portal/index/news', ['id' => $data['id']]);
            $routeModel->deleteRoute('portal/index/news_info', ['cid' => $data['id']]);
        }
...
tpl;
        ...
        $categoryTree = $portalCategoryModel->adminCategoryTableTree(8,$selectedIds, $tpl);
